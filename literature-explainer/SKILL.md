...
- `问答摘要` **必须** 是核心章节，并按轮次固定四项：
  ...
  - 证据原文
  - 未解决点/后续动作
- 生成完成后写回一条 `stage=note` 记录；若该论文已有旧的 note 记录，**必须**覆盖旧记录，而不是新增第二条。
...
- **必须** 已确认本次写回覆盖的是该论文唯一 `stage=note` 记录与本轮唯一最终 note artifact。
...
- **必须** 在同一篇论文的 memory 文件中只保留一条 `stage=note` 记录。
- note 生成的数据范围固定为：
