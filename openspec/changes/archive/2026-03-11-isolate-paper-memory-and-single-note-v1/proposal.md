...
- 将记忆存储改为按论文拆分文件，并明确 note 内容不写回 memory。
...
### New Capabilities
- `paper-memory-isolation`: 以论文级 key 隔离 dispatch 产物、记忆文件和最终学习笔记，并保证每篇论文唯一 note 文件。
...
- 影响 `literature-explainer/scripts/memory_engine.py` 的默认存储路径与 note 阶段读取语义。
